...
# NO BACKEND IMPORTS (except Engine calls via session state)
    ...
    with st.sidebar:
        ...
        with st.expander("Import Data", expanded=not bool(active_ds)):
            ...
            if not loaders:
                ...
                selected_source = st.selectbox("Source Type", list(loader_map.keys()), key="imp_src")
                loader = loader_map[selected_source]
                
                # Dynamic Form
                params = render_schema_fields(loader.get('ui_schema', []), key_prefix=f"load_{loader['name']}")
                ...
                if st.button("Load Data", type="primary", key=f"btn_load_{loader['name']}", use_container_width=True):
                    if not alias_val:
                        ...
                        lf = engine.run_loader(loader['name'], params)
                        if lf is not None:
        ...
        if dataset_names:
             ...
             for name in dataset_names:
                ...
                if c2.button("🗑️", key=f"del_{name}"):
                    ...
                    if name in st.session_state.all_recipes: del st.session_state.all_recipes[name]
        ...
        registry = engine.get_supported_steps()
        if registry:
            # 1. Group Steps
            grouped_steps = {} 
            for step_type in registry:
                meta = engine.get_step_metadata(step_type)
                label = meta.get("label", step_type)
                group = meta.get("group", "Misc")
                if group not in grouped_steps: grouped_steps[group] = []
                grouped_steps[group].append((step_type, label))
            
            # 2. Render Groups as Tabs
            preferred_order = ["Columns", "Rows", "Combine"]
            sorted_groups = sorted(grouped_steps.keys(), key=lambda x: preferred_order.index(x) if x in preferred_order else 99)
            
            if sorted_groups:
                tabs = st.tabs(sorted_groups)
                for tab, group in zip(tabs, sorted_groups):
                    with tab:
                        steps = grouped_steps[group]
                        # Dropdown Selection
                        options_map = {label: step_type for step_type, label in steps}
                        selected_label = st.selectbox(f"Operation", list(options_map.keys()), key=f"sel_grp_{group}", label_visibility="collapsed")
                        
                        if st.button(f"Add Step", key=f"add_btn_{group}", type="primary", use_container_width=True):
        ...
        with st.expander("🔪 Recipe Actions", expanded=False):
            ...
            # st.code removed as per user request
            
            st.download_button("💾 Download JSON", recipe_json, "recipe.json", "application/json", use_container_width=True)
