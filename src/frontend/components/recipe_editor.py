...
from src.frontend.renderers import render_step_ui
    ...
    for i, step in enumerate(st.session_state.recipe_steps):
        ...
        if current_schema:
            try:
                ...
                # Convert Schema object to dict for pl.LazyFrame
                ...
                # We use the potentially updated params
                next_lf = engine.apply_step(dummy_lf, step)

                # 3. Get New Schema
                current_schema = next_lf.collect_schema()
            except Exception as e:
                # If simulation fails (e.g. invalid params), we effectively stop propagation or keep previous?
                # Keeping previous might be confusing if cols were dropped.
                # Let's keep current_schema as is to allow user to fix the step without UI crashing.
                # Or maybe we shouldn't update it?
                # print(f"Schema Propagation Fail: {e}")
                pass
