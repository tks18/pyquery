"""
Project Dialog Component for PyQuery.

Provides UI dialogs for project export and import functionality.
"""
...
def render_project_section():
    ...
    col_save.button("💾", help="Save Project (.pyquery)", key="btn_proj_save", on_click=open_export_dialog)
    col_load.button("📂", help="Load Project (.pyquery)", key="btn_proj_load", on_click=open_import_dialog)
...
def show_project_export_dialog(engine: PyQueryEngine):
    ...
    def export_dialog():
        ...
        st.info(f"📦 This project contains **{len(dataset_names)} dataset(s)**: {', '.join(dataset_names)}")
        ...
        if path_mode == "Relative Paths":
            st.caption("Relative paths will be calculated from the base directory.")
            
            col_base, col_pick = st.columns([0.8, 0.2])
            base_dir = col_base.text_input(
                "Base Directory",
                value=st.session_state.get("proj_export_base_dir", os.getcwd()),
            ...
            col_pick.button("📁", on_click=on_pick_base, help="Browse for folder")
        ...
        if active_ds:
            dataset_meta = engine.get_dataset_metadata(active_ds)
        ...
        def on_pick_output():
            ...
            if picked:
                st.session_state.proj_export_path = os.path.join(picked, default_filename)
        
        col_pick.button("📁", on_click=on_pick_output, key="btn_pick_export", help="Browse for folder")
        ...
        if col_export.button("💾 Export Project", type="primary", use_container_width=True):
            ...
            try:
                with st.spinner("Exporting project..."):
                    ...
                    saved_path = engine.save_project_to_file(
...
def show_project_import_dialog(engine: PyQueryEngine):
    ...
    def import_dialog():
        ...
        if uploaded_file:
            ...
            current_datasets = engine.get_dataset_names()
            if current_datasets and import_mode == "Replace All":
                st.warning(f"⚠️ This will replace {len(current_datasets)} existing dataset(s): {', '.join(current_datasets)}")
            ...
            try:
                with st.spinner("Importing project..."):
                    ...
                    try:
                        ...
                        result = engine.load_project_from_file(tmp_path, mode=mode)
                        ...
                        # Show results
                        if result.datasets_loaded:
                            st.success(f"✅ Loaded {len(result.datasets_loaded)} dataset(s): {', '.join(result.datasets_loaded)}")
