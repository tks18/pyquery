...
def show_sql_loader(engine: PyQueryEngine, edit_mode: bool = False, edit_dataset_name: Optional[str] = None):
    ...
    if edit_mode and edit_dataset_name and not st.session_state.get(edit_init_key):
        ...
        params = meta.loader_params if meta and meta.loader_params else {}
    ...
    auto_infer = st.checkbox(
        "✨ Auto Detect & Clean Types", value=False, key="dlg_sql_infer")
    ...
    if c_submit.button(btn_label, type="primary", width="stretch"):
        ...
        params = {"conn": conn, "query": query,
                  "alias": alias_val, "auto_infer": auto_infer}

        with st.spinner("Executing query & loading..."):
            res = engine.io.run_loader("Sql", params)
            if res:
                lf_or_lfs, meta = res
                ...
                engine.datasets.add(alias_val, lf_or_lfs, meta,
                                    loader_type="SQL",
