"""
Analytics step renderers - Class-based UI components for analytics operations.
...
class TimeBinStep(BaseStepRenderer[TimeBinParams]):
    """Renderer for the time_bin step - bins timestamps into intervals."""

    def render(self, step_id: str, params: TimeBinParams,
               schema: Optional[pl.Schema]) -> TimeBinParams:
        c1, c2 = st.columns(2)
...
class RollingAggStep(BaseStepRenderer[RollingAggParams]):
    """Renderer for the rolling_agg step - rolling window aggregations."""

    def render(self, step_id: str, params: RollingAggParams,
               schema: Optional[pl.Schema]) -> RollingAggParams:
        current_cols = schema.names() if schema else []
...
class NumericBinStep(BaseStepRenderer[NumericBinParams]):
    """Renderer for the numeric_bin step - bins numeric values."""

    def render(self, step_id: str, params: NumericBinParams,
               schema: Optional[pl.Schema]) -> NumericBinParams:
        c1, c2 = st.columns(2)
        current_cols = schema.names() if schema else []
        col_val = params.col if params.col in current_cols else (
            current_cols[0] if current_cols else "")
...
class MathOpStep(BaseStepRenderer[MathOpParams]):
    """Renderer for the math_op step - mathematical operations."""

    def render(self, step_id: str, params: MathOpParams,
               schema: Optional[pl.Schema]) -> MathOpParams:
        c1, c2 = st.columns(2)
        current_cols = schema.names() if schema else []
        col_val = params.col if params.col in current_cols else (
            current_cols[0] if current_cols else "")
...
class DateExtractStep(BaseStepRenderer[DateExtractParams]):
    """Renderer for the date_extract step - extracts date parts."""

    def render(self, step_id: str, params: DateExtractParams,
               schema: Optional[pl.Schema]) -> DateExtractParams:
        c1, c2 = st.columns(2)
        current_cols = schema.names() if schema else []
        col_val = params.col if params.col in current_cols else (
            current_cols[0] if current_cols else "")
...
class CumulativeStep(BaseStepRenderer[CumulativeParams]):
    """Renderer for the cumulative step - cumulative operations."""

    def render(self, step_id: str, params: CumulativeParams,
               schema: Optional[pl.Schema]) -> CumulativeParams:
        c1, c2, c3 = st.columns(3)
        current_cols = schema.names() if schema else []
        col_val = params.col if params.col in current_cols else (
            current_cols[0] if current_cols else "")
...
class RankStep(BaseStepRenderer[RankParams]):
    """Renderer for the rank step - ranking operations."""

    def render(self, step_id: str, params: RankParams,
               schema: Optional[pl.Schema]) -> RankParams:
        c1, c2, c3 = st.columns(3)
        current_cols = schema.names() if schema else []
        col_val = params.col if params.col in current_cols else (
            current_cols[0] if current_cols else "")
...
class DiffStep(BaseStepRenderer[DiffParams]):
    """Renderer for the diff step - difference/percentage change operations."""

    def render(self, step_id: str, params: DiffParams,
               schema: Optional[pl.Schema]) -> DiffParams:
        c1, c2, c3 = st.columns(3)
        current_cols = schema.names() if schema else []
        col_val = params.col if params.col in current_cols else (
            current_cols[0] if current_cols else "")
...
class ZScoreStep(BaseStepRenderer[ZScoreParams]):
    """Renderer for the z_score step - standardization/z-score calculation."""

    def render(self, step_id: str, params: ZScoreParams,
               schema: Optional[pl.Schema]) -> ZScoreParams:
        current_cols = schema.names() if schema else []

        c1, c2 = st.columns(2)
        idx = current_cols.index(
...
class SkewKurtStep(BaseStepRenderer[SkewKurtParams]):
    """Renderer for the skew_kurt step - skewness/kurtosis calculation."""

    def render(self, step_id: str, params: SkewKurtParams,
               schema: Optional[pl.Schema]) -> SkewKurtParams:
        current_cols = schema.names() if schema else []
