"""
Cleaning step renderers - Class-based UI components for data cleaning operations.
...
from pyquery_polars.core.params import (
    ...
    TextExtractDelimParams, RegexToolParams, NormalizeSpacesParams, SmartExtractParams,
    CleanTextParams, MaskPIIParams, AutoImputeParams, CheckBoolParams
)


class FillNullsStep(BaseStepRenderer[FillNullsParams]):
    """Renderer for the fill_nulls step - fills null values with various strategies."""

    def render(self, step_id: str, params: FillNullsParams,
               schema: Optional[pl.Schema]) -> FillNullsParams:
        c1, c2 = st.columns([0.5, 0.5])

        params.strategy = c1.selectbox(
            "Strategy",
            ["forward", "backward", "mean", "median",
                "min", "max", "zero", "literal"],
...
class RegexExtractStep(BaseStepRenderer[RegexExtractParams]):
    """Renderer for the regex_extract step - extracts patterns using regex."""

    def render(self, step_id: str, params: RegexExtractParams,
               schema: Optional[pl.Schema]) -> RegexExtractParams:
        current_cols = schema.names() if schema else []
...
class StringCaseStep(BaseStepRenderer[StringCaseParams]):
    """Renderer for the string_case step - changes string case (upper/lower/title/trim)."""

    def render(self, step_id: str, params: StringCaseParams,
               schema: Optional[pl.Schema]) -> StringCaseParams:
        current_cols = schema.names() if schema else []
...
class StringReplaceStep(BaseStepRenderer[StringReplaceParams]):
    """Renderer for the string_replace step - replaces substrings."""

    def render(self, step_id: str, params: StringReplaceParams,
               schema: Optional[pl.Schema]) -> StringReplaceParams:
        current_cols = schema.names() if schema else []
        ...
        c1, c2 = st.columns(2)
        params.pat = c1.text_input(
            "Find", value=params.pat, key=f"sr_p_{step_id}")
...
class DropNullsStep(BaseStepRenderer[DropNullsParams]):
    """Renderer for the drop_nulls step - removes rows with null values."""

    def render(self, step_id: str, params: DropNullsParams,
               schema: Optional[pl.Schema]) -> DropNullsParams:
...
class TextSliceStep(BaseStepRenderer[TextSliceParams]):
    """Renderer for the text_slice step - extracts substring by position."""

    def render(self, step_id: str, params: TextSliceParams,
               schema: Optional[pl.Schema]) -> TextSliceParams:
        c1, c2 = st.columns(2)
        current_cols = schema.names() if schema else []
        col_val = params.col if params.col in current_cols else (
            current_cols[0] if current_cols else "")
...
class TextLengthStep(BaseStepRenderer[TextLengthParams]):
    """Renderer for the text_length step - calculates string length."""

    def render(self, step_id: str, params: TextLengthParams,
               schema: Optional[pl.Schema]) -> TextLengthParams:
        current_cols = schema.names() if schema else []
        c1, c2 = st.columns(2)
        col_val = params.col if params.col in current_cols else (
            current_cols[0] if current_cols else "")
...
class StringPadStep(BaseStepRenderer[StringPadParams]):
    """Renderer for the string_pad step - pads strings to target length."""

    def render(self, step_id: str, params: StringPadParams,
               schema: Optional[pl.Schema]) -> StringPadParams:
        c1, c2 = st.columns(2)
        current_cols = schema.names() if schema else []
...
class TextExtractDelimStep(BaseStepRenderer[TextExtractDelimParams]):
    """Renderer for the text_extract_delim step - extracts text between delimiters."""

    def render(self, step_id: str, params: TextExtractDelimParams,
               schema: Optional[pl.Schema]) -> TextExtractDelimParams:
        c1, c2 = st.columns(2)
        current_cols = schema.names() if schema else []
...
class RegexToolStep(BaseStepRenderer[RegexToolParams]):
    """Renderer for the regex_tool step - advanced regex operations."""

    def render(self, step_id: str, params: RegexToolParams,
               schema: Optional[pl.Schema]) -> RegexToolParams:
        current_cols = schema.names() if schema else []
...
class NormalizeSpacesStep(BaseStepRenderer[NormalizeSpacesParams]):
    """Renderer for the normalize_spaces step - normalizes whitespace."""

    def render(self, step_id: str, params: NormalizeSpacesParams,
               schema: Optional[pl.Schema]) -> NormalizeSpacesParams:
        current_cols = schema.names() if schema else []
...
class SmartExtractStep(BaseStepRenderer[SmartExtractParams]):
    """Renderer for the smart_extract step - extracts email/url/ip patterns."""

    def render(self, step_id: str, params: SmartExtractParams,
               schema: Optional[pl.Schema]) -> SmartExtractParams:
        c1, c2 = st.columns(2)
        current_cols = schema.names() if schema else []
...
class CleanTextStep(BaseStepRenderer[CleanTextParams]):
    """Renderer for the clean_text step - comprehensive text cleaning."""

    def render(self, step_id: str, params: CleanTextParams,
               schema: Optional[pl.Schema]) -> CleanTextParams:
        current_cols = schema.names() if schema else []
...
class MaskPIIStep(BaseStepRenderer[MaskPIIParams]):
    """Renderer for the mask_pii step - masks personally identifiable information."""

    def render(self, step_id: str, params: MaskPIIParams,
               schema: Optional[pl.Schema]) -> MaskPIIParams:
        c1, c2 = st.columns(2)
        current_cols = schema.names() if schema else []
        ...
        idx = type_options.index(
            params.type) if params.type in type_options else 0
...
class AutoImputeStep(BaseStepRenderer[AutoImputeParams]):
    """Renderer for the auto_impute step - intelligent missing value imputation."""

    def render(self, step_id: str, params: AutoImputeParams,
               schema: Optional[pl.Schema]) -> AutoImputeParams:
        c1, c2 = st.columns(2)
        current_cols = schema.names() if schema else []
...
class CheckBoolStep(BaseStepRenderer[CheckBoolParams]):
    """Renderer for the check_bool step - converts values to boolean."""

    def render(self, step_id: str, params: CheckBoolParams,
               schema: Optional[pl.Schema]) -> CheckBoolParams:
        current_cols = schema.names() if schema else []
