...
def setup_parser():
    """
    Defines the argument parser and subparsers for the CLI.
...
def launch_headless(args, ui, theme_manager):
    """Executes the headless pipeline."""
    if args.dev:
        ui.log_step("Dev Mode Enabled (Headless)",
                    module="DEV-MODE", icon="🛠️")
    run_headless(args, ui, theme_manager)


def launch_ui(args, ui):
    """Launches the Streamlit UI."""
    ui.reset_timer()
    ui.log_step(
        f"Launching Streamlit on port {args.port}...", module="WEB-UI", icon="🌊")

    # Resolve path relative to THIS file's location in src/pyquery_polars/cli
    current_dir = os.path.dirname(os.path.abspath(__file__))
    package_root = os.path.dirname(current_dir)
    app_path = os.path.join(package_root, "frontend", "app.py")

    if not os.path.exists(app_path):
        ui.log_error("Frontend App Not Found", f"Path: {app_path}")
        sys.exit(1)
    ...
    if args.dev:
        ...
        ui.log_step("Dev Mode Enabled: Verbose Logging Active",
                    module="DEV-MODE", icon="🛠️")
    try:
        subprocess.run(cmd)
    except KeyboardInterrupt:
        ui.log_step("Streamlit Server stopped.", module="Shutdown", icon="🛑")
        sys.exit(0)


def launch_api(args, ui):
    ...
    try:
        ...
        ui.log_step("API Server stopped.", module="Shutdown", icon="🛑")
        sys.exit(0)
