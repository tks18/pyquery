...
import polars as pl
from pyquery.core.models import StepMetadata, TransformContext
...
class StepRegistry:
    ...
    @classmethod
    def register_renderer(cls, step_type: str, frontend_func: FrontendFunc):
        """
        Attach a renderer to an existing step definition.
        """
        if step_type in cls._steps:
            cls._steps[step_type].frontend_func = frontend_func
        else:
            # We could warn, but maybe backend isn't loaded yet?
            # ideally backend should be loaded first.
            pass
